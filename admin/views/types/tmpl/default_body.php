...
<?php foreach($this->items as $i => $item):
	...
	$canEdit        = $user->authorise('core.edit',			'com_velo.type.'.$item->id);
	$canCheckin     = $user->authorise('core.manage',		'com_checkin') || $item->checked_out == $userId || $item->checked_out == 0;
	//$canEditOwn     = $user->authorise('core.edit.own',		'com_velo.chantier.'.$item->id) && $item->created_by == $userId;
	$canChange      = $user->authorise('core.edit.state',	'com_velo.type.'.$item->id) && $canCheckin;
	?>
	<tr class="row<?php echo $i % 2; ?>">
		...
		<td>
		    <a href="<?php echo JRoute::_('index.php?option=com_velo&view=type&layout=edit&id=' . $item->id); ?>"><?php echo $item->label_tr; ?></a> <small>(<?php echo $item->label_id; ?>)</small>
		</td>
		<td class="center">
